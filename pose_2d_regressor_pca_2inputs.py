...
class PoseRegressor(nn.Module):
    def __init__(self, input_dim=68, output_dim=8, input_projection_dim=256):
...
class PoseRegressionModel:
    def __init__(self, encoders, input_dim=68, output_dim=8, learning_rate=0.001):
        ...
        self.writer = SummaryWriter(f'poseregressor_pca_2inputs_runs/{self.modelname}')
        self.knn = FastKNN()
    ...
    def plot_and_compare_skeletons(self, skeleton, skeleton2, nearest_skeleton, predicted_embeddings, target, step):
        ...
        add_index_labels(axs[0, 0], skeleton2)
    ...
    def train(self, train_dataset, val_dataset, batch_size=128, num_epochs=300, patience=10):
        ...
        for epoch in range(num_epochs):
            ...
            for batch_idx, batch in enumerate(progress_bar):
                ...
                # Calculate loss for each body part
                loss_upper = self.criterion(outputs[:, upper_indices], targets[:, upper_indices])
                loss_lower = self.criterion(outputs[:, lower_indices], targets[:, lower_indices])
                loss_l_arm = self.criterion(outputs[:, l_arm_indices], targets[:, l_arm_indices])
                loss_r_arm = self.criterion(outputs[:, r_arm_indices], targets[:, r_arm_indices])

                # Amplify the loss for lower body embeddings
                loss = loss_upper + 2 * loss_lower + loss_l_arm + loss_r_arm
            ...
            if val_loss < best_val_loss:
                ...
                epochs_no_improve = 0
                self.save_model(f'poseregressor_pca_runs/{self.modelname}/best_.pth', f'poseregressor_pca_runs/{self.modelname}/best_encoders.pkl')
            else:
                epochs_no_improve += 1
        ...
        self.save_model(f'poseregressor_pca_runs/{self.modelname}/_best_model.pth', f'{self.modelname}_best_model_encoders.pkl')
        self.writer.close()
    ...
    def save_model(self, filename, encoders_filename):
        os.makedirs(os.path.dirname(filename), exist_ok=True)
        torch.save(self.model.state_dict(), filename)
        with open(encoders_filename, 'wb') as f:
            pickle.dump(self.encoders, f)
    
    def load_model(self, model_filename, encoders_filename):
        ...
        if not os.path.exists(encoders_filename):
            raise FileNotFoundError(f"Encoders file not found: {encoders_filename}. "
...
if __name__ == "__main__":
    ...
    model = PoseRegressionModel(encoders=train_dataset.encoders, input_dim=68, output_dim=8)
