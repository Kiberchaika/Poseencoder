...
from ddpm import MultiposeDiffusion, instantiate_ddpm_from_config, reshape_conditioning
...
class DDIMSampler(nn.Module):
    def __init__(self, model, beta_start_end, n_T, device):
        ...
        for i in range (0, 250):
            steps_ranges.append(i)
        for steps in steps_ranges:  # Add or remove steps as needed
...
def generate_samples_and_video(ddim_sampler, conditioning_input, device, n_frames=15, n_sample=1, w=1.5, steps=10, eta=1.0):
    ...
    with torch.no_grad():
        for j in range(n_frames):
            ...
            conditioning = reshape_conditioning(conditioning_input)
            x_gen = sample_ddim(ddim_sampler, n_sample, (17, 3), device, guide_w=w, conditioning=conditioning.to(device), steps=steps, eta=eta)
            end_time = time.time()
            ...
            # Only drawing the first sample of the batch
            fig = visualize_3d_and_2d_skeletons(x_gen[0].cpu().numpy(), conditioning_input[0].cpu().numpy())
...
if __name__ == '__main__':
    ...
    # Initialize model
    # ddpm = MultiposeDiffusion(nn_model=ContextPoseUnet(in_features=17*3, n_feat=n_feat), betas=(1e-4, 0.02), n_T=n_T, device=device, drop_prob=0.1)
    # ddpm.to(device)

    ddpm = instantiate_ddpm_from_config("/home/k4/Projects/Poseencoder/MultiposeDiffusion/pose_ddpm_runs/batchsize_256_nfeat_512/config.gin", 
                                        device, 
                                        "/home/k4/Projects/Poseencoder/MultiposeDiffusion/pose_ddpm_runs/batchsize_256_nfeat_512/model_70.pth")
    ...
    generate_samples_and_video(ddim_sampler, conditioning_input, device, 30, w=1.0, steps=25)
